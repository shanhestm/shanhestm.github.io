...
(function($) {
	...
	FlipClock.DailyCounterFace = FlipClock.Face.extend({
		...
		build: function(excludeHours, time) {
			...
			if(this.showSeconds) {
				$(this.createDivider('秒')).insertBefore(this.factory.lists[this.factory.lists.length - 2].$el);
			}
			else
			{
				offset = 2;
			...
			$(this.createDivider('分')).insertBefore(this.factory.lists[this.factory.lists.length - 4 + offset].$el);
			$(this.createDivider('时')).insertBefore(this.factory.lists[this.factory.lists.length - 6 + offset].$el);
			$(this.createDivider('日', true)).insertBefore(this.factory.lists[0].$el);
